...
namespace Core.Editor
{
    public class NodeGraphEditorWindow : EditorWindow, ISearchWindowProvider
    {
        ...
        {
            // Tansparent icon to trick search window into indenting items
        ...
        public List<SearchTreeEntry> CreateSearchTree(SearchWindowContext context)
        {
            ...
            Type[] types = AppDomain.CurrentDomain.GetAssemblies().SelectMany(assembly=>assembly.GetTypes()).Where(type => typeof(NodeView).IsAssignableFrom(type) && type.IsClass && !type.IsAbstract && type != typeof(NodeView)&& type != typeof(ResultNodeView)).ToArray();
            ...
            // Firstitem in the tress is the title of the window.
            ...
            foreach (var nodeEntry in nodeEntries)
            {
                ...
                for (var i = 0; i < nodeEntry.title.Length - 1; i++)
                {
                    ...
                    if (i >= groups.Count)
                    {
                        ...
                        // rest of the group from the current entry.
                        createIndex = i;
                        break;
                    }
                    if (groups[i] != group)
                    {
                        ...
                        // so we remove everyfrom from the point where if doesn't match anymore, and the add the rest
                ...
                for (var i = createIndex; i < nodeEntry.title.Length - 1; i++)
                {
                    var group = nodeEntry.title[i];
                    groups.Add(group);
                    tree.Add(new SearchTreeGroupEntry(new GUIContent(group)) {  level = +1 });
                }
